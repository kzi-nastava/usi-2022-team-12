...
namespace HealthInstitution.Core.Features.AppointmentScheduling.Services
{
    public class RecommendationService : IRecommendationService
    {
        ...
        public RecommendationService(ISchedulingService schedulingService, IDoctorService doctorService, IAppointmentUpdateRequestRepository appointmentUpdateRequestRepository, IRoomService roomService)
        {
        ...
        public Appointment FindFreeAppointmentForDoctorInTimeSpan(Patient patient, List<Doctor> doctors, DateTime intervalStart, DateTime intervalEnd)
        {
            for (DateTime potentialStart = intervalStart; potentialStart.AddMinutes(15) <= intervalEnd; potentialStart = potentialStart.AddMinutes(1))
            {
                ...
                {
                    if (!_schedulingService.IsDoctorAvailable(doctor, appointmentStart, appointmentEnd) ||
                        !_appointmentUpdateRequestRepository.IsDoctorAvailable(doctor, appointmentStart, appointmentEnd))
                        continue;

                    Room freeRoom = _schedulingService.FindFreeRoom(RoomType.ExaminationRoom, appointmentStart, appointmentEnd);
                    if (freeRoom == null)
                        continue;
